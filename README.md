...
| RAG 방식 | PDF page-aware chunk + lexical/embedding hybrid retrieval |
| 개인화 | 회상 점수, 자기 설명 품질, 힌트 의존도, 오개념 빈도, 숙련도 기반 추천 |
| 데이터베이스 | PostgreSQL + Alembic, SQLite 테스트 fallback |
| 시연 흐름 | PDF 업로드 → 개념 추출 → 질문 → 답변 평가 → 힌트 → 자기 설명 → 복습/대시보드 |
...
flowchart LR
  ...
  API --> Retrieval[RAG Retrieval Service<br/>Lexical + Embedding]
...
| Dashboard | 추천 학습법, 메모리 강도, 약한 개념, 오개념 노트, 복습 일정, 최근 세션 |
| Study Room | PDF 업로드부터 질문, 답변 평가, 힌트, 자기 설명, 리포트까지 진행 |
| Daily Review | 오늘 복습할 개념과 추천 이유 표시 |
...
8. 자기 설명 평가
9. Daily Review와 Dashboard에서 개인화 결과 확인
